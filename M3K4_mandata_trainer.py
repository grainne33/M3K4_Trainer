...
class FileDecoderApp:
    ...
    def create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="M3K4武将修改器",
                                font=("Arial", 16, "bold"))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 10))

        # 分隔线
        separator = ttk.Separator(main_frame, orient=tk.HORIZONTAL)
        separator.grid(row=1, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
        ...
        self.status_label.grid(row=5, column=0, columnspan=3, sticky=tk.W, pady=(5, 10))

        # 分隔线
        separator2 = ttk.Separator(main_frame, orient=tk.HORIZONTAL)
        separator2.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)

        # 角色选择区域
        character_frame = ttk.LabelFrame(main_frame, text="选择角色", padding="5")
        character_frame.grid(row=7, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
    ...
    def get_weapon_name(self, weapon_code):
        """根据武器代码获取武器名称"""
        return WEAPON_MAP.get(weapon_code, f"未知武器({weapon_code:02X})")

    def get_skill_info(self, skill_value):
        """根据特技数值获取特技名称和解释"""
        if skill_value in SKILL_MAP:
            name, desc = SKILL_MAP[skill_value]
            return f"{name}({desc})"
        else:
    ...
    def get_attack_info(self, attack_code):
        ...
        if attack_code == 0xFF:
            return "无"
        elif attack_code in ATTACK_MAP:
            attack = ATTACK_MAP[attack_code]
            min_damage = attack[3]
    ...
    def parse_block(self, block_data, block_index):
        ...
        if len(identity_bytes) >= 4:
            ...
            block['identity'] = IDENTITY_MAP.get(identity_value, "武将")

        # 第29-32字节: 归属
        belonging_bytes = block_data[28:32]
        if len(belonging_bytes) >= 4:
            belonging_value = struct.unpack('<I', belonging_bytes)[0]
            block['belonging'] = ALLEGIANCE_MAP.get(belonging_value, f"未知({belonging_value:02X})")
        ...
        if len(position_bytes) >= 4:
            ...
            if position_value in POSITION_MAP:
                block['position'] = POSITION_MAP[position_value]
            elif (position_value & 0xFFFFFF00) == 0x00000000:
                city_code = position_value & 0xFF
                block['position'] = f"驻守城池(代码:{city_code:02X})"
            else:
